...
Usage:
    ...
    python demo.py --visualize     # With matplotlib plots
    python demo.py --steps 500     # Custom step count
    python demo.py --multi-agent   # Multi-agent competition
...
def run_multi_agent(
    ...
    if verbose:
        ...
        for rank, result in enumerate(comparison['rankings'], 1):
            ...
            print(f"{medal} Rank {rank}: {preset_name}")
...
def print_statistics(stats: Dict):
    ...
    print(f"   Mean φ: {stats['avg_phi']:.3f} ± {stats['std_phi']:.3f}")
    ...
    print(f"   φ ↔ Policy correlation: {stats['corr_phi_policy']:.3f}\n")
    ...
    print(f"   φ ↔ Energy: {stats['corr_phi_energy']:.3f}\n")


def visualize_results(nexus: OmniNexus, history: List[Dict]):
    ...
    ax.set_title('Oscillator Phase (φ)', fontweight='bold')
    ax.set_xlabel('Step')
    ax.set_ylabel('φ')
    ...
    ax.set_title('Phase Space (φ vs Policy)', fontweight='bold')
    ax.set_xlabel('φ')
