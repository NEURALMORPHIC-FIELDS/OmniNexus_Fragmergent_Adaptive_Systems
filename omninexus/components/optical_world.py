...
class OpticalWorld:
    ...
    def __init__(
        ...
        # Cache filter kernel for optimization
        self._filter_kernel: Optional[np.ndarray] = None
        self._cached_size: Optional[Tuple[int, int]] = None

    def _get_filter_kernel(self) -> np.ndarray:
        """Get or create cached filter kernel."""
        if self._filter_kernel is None or self._cached_size != self.size:
            x = np.arange(self.size[0])[:, None]
            y = np.arange(self.size[1])[None, :]
            dist_sq = x**2 + y**2
            self._filter_kernel = np.exp(-self.complexity * dist_sq)
    ...
    def generate(self, seed: Optional[int] = None) -> np.ndarray:
        ...
        # Get cached filter kernel
        filter_kernel = self._get_filter_kernel()
        ...
        if world_max > world_min:
            self.state = ((world - world_min) / (world_max - world_min)).astype(np.float32)
        else:
            self.state = np.zeros_like(world, dtype=np.float32)
